...
def build_epinions_digraph_from_local(path=LOCAL_PATH):
    G = DiGraph()
    with open(path, mode="rt", encoding="utf-8", errors="replace") as fh:
        for line in fh:
            line = line.strip()
...
def get_distance_in_graph(trust_graph, truster, trustee):
    try:
        dist = shortest_path_length(trust_graph, source=truster, target=trustee)
...
def required_length(topK: int, alpha: float) -> int:
    ...
    res = c / (1 - a) * pow(float(topK), a) * pow(N, 1 - a)
    return round(res)
...
def sample_datarow(graph, n_bots, req_length):
    ...
    Independent_pre_ranks_freenet = rankdata(
        Independent_pre_scores_freenet, method="min"
    )

    Independent_post_scores_freenet = [
        compute_wot_score_indep(graph_post, victim, trustee) for trustee in range(N)
    ]
    Independent_post_ranks_freenet = rankdata(
        Independent_post_scores_freenet, method="min"
    )
